            ...
            if (response.success === false) {
                ...
                for (i = 0; i < arr[0].length; i++) {
                    html = '<li id="inviteItem' + arr[0][i].ID + '"> <div class="notfiyListitem">'
                         ...
                         + '<button class="btn btn-primary btn-sm" onclick="acceptProject(' + arr[0][i].ID + ')">Accept</button>'
                         + '<button class="btn btn-default btn-sm" onclick="declineProject(' + arr[0][i].ID + ')">Decline</button>'
                         + '</div></li>';
                    $('#inviteDropDown').append(html);
                }
            }
